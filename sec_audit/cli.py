...
def run_from_args(args: SimpleNamespace) -> None:
    ...
    if getattr(args, "patch", False):
        try:
            ...
            # It is never passed as a CLI flag to avoid exposure in shell history
 
            generator  = PatchGenerator(use_llm=use_llm)
            ...
            for p in patches:
                source = "Claude AI" if p.is_llm else "template"
                print(f"  [{p.severity:<8}] {p.filename:<35} ({source})")
            print(f"\n  README: {patch_dir}/README.md")
            if llm_count:
                print(f"  {llm_count} patch(es) generated by Claude, {tmpl_count} from templates")
            print()
        except Exception as e:
